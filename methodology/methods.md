...
### *Methodology*
In order to create Least Cost Paths (LCPs) as accurately as possible for this project
I created a project model in QGIS. QGIS does provide its own least cost path analysis plugin,
however it only generates LCPs based on raster layers like an elevation map. This is a
very basic method that yields results that are acceptable for smaller projects and simple cases
where it is only a small factor. This project required a method that could generate more
accurate results since the main component and factor that is being analyzed here are the
LCPs generated based on the topographical information. Therefore a least cost path
algorithm needed to be created that used other algorithms from R (computer programming language)
and different raster layers that contribute to cost analysis and walk times/cost.
This called for the use of r.walk, r.drain, as well as digital elevation raster layer and
...
points that were specified in the inputs, and then this will be turned to be used in the next
algorithm, r.drain.

R.drain uses the cumulative cost layer and the movement direction layer
that was generated from r.walk algorithm. R.drain creates a least cost path essentially as
...
means that it would create a least cost path because tracing the flow of water or a liquid
would show the quickest and simplest path the water would flow down a hill or mountain.
...
#### **Roman Britain**
The maps here represent the three different ways the LCPs were calculated: \\
...
the Roman roads were for military use. They helped to transport armies as wells as
...
and settlements. This helped to explain why some LCPs generated display different
...
were constructed following the flow of the landscape to make for easier most cost
effective transport or if they were just concerned with the shortest path regardless
of terrain and topographical influence.

There are several maps that represent different LCPs that were generated with the
method mentioned above. While there was the same thought of analysis as the LCPs
generated for the area in Germany, there is more analysis here on topographical influences
of roads versus cultural influences on the roads. This was accomplish by using the
research from Joseph Lewis and Hector A. Orengo. I used their research and maps that
...
Their maps that they generated are overlapped onto my maps with my generated LCPs.
...
were more influenced by social aspects.
